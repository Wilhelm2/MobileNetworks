...
for j in intervals:
	for nbHosts in hosts:
		...
		with open("data/"+str(dimension) +"/interval"+ str(j) +"Hosts" +str(nbHosts) + "/nbSentMessagesFile.dat",'r') as csvfile:
			plots = csv.reader(csvfile, delimiter=' ')
			for row in plots:
				xtime.append(float(row[0]))
				yTotalSentMessages.append(float(row[1]))
				yWiredSentMessages.append(float(row[2]))
				yWifiSentMessages.append(float(row[3]))
		...
		plt.ylabel('Sent messages', fontsize=18)
...
#		plt.xlim(xmin=0,xmax=300)
		...
		plt.savefig("Graphes/"+str(dimension) +"/interval"+ str(j) +"Hosts" +str(nbHosts) + "/nbSentMessagesFile.eps", format='eps', bbox_inches="tight")
