...
void BroadcastController::HostSystemJoin(unsigned int idMH, unsigned int idMSS) {
    ...
    unsigned int localSeq =
        HostsClock[idMH]
                  [idMH];  // to save the case where idMH broadcasted a message prior to connecting to the first station
...
bool BroadcastController::entrySatisfiesDeliveryCondition(unsigned int index,
                                                          unsigned int idSource,
                                                          unsigned int dependencyValue,
                                                          unsigned int value) {
    return dependencyValue == 0 || (value >= (dependencyValue - (index == idSource)));  // optimization
    //    return (localVectorEntry >= toDeliverEntry && index != idSource) || (localVectorEntry >= (toDeliverEntry-1) &&
    //    index == idSource);
}
...
void BroadcastController::DeliveryError(string errorReason,
                                        idMsg idM,
...
// Called by a station whenever it removes a message from its SBuffer, ie when all hosts connected to it delivered the
// message
