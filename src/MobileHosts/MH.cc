        ...
        if (isMessageFromCell(m)) {
            if (state.isConnectionEstablished()) {
                if (dynamic_pointer_cast<const AppS_h>(m) != nullptr)
                    RecvAppS_h(dynamic_pointer_cast<const AppS_h>(m));
                else if (dynamic_pointer_cast<const Ack_S>(m) != nullptr)
...
void MH::SendAck(cMessage* msg) {
    if (!state.isInitiatingConnection())  // required because host might change station in chooseStation called just
                                          // before
    {
...
void MH::RecvAppS_h(const inet::IntrusivePtr<const AppS_h>& msg) {
    struct s_recvMsgMH mt = {
        msg->getSeq_S(), msg->getId(),
        (find(msg->getM_d().begin(), msg->getM_d().end(), id_h) !=
...
void MH::FIFODeliver() {
    ...
    if (cpt > 50 &&
        sendSeqTimeout
            .isScheduled())  // delivered many messages and is currently sending ACK messages (i.e., not in handoff)
    {
        sendPacket(
            new Packet("ack_h", MessageFactory::createAck_h(id_h, RBuffer.getAckIntervals(seqNC), seqNC, Ses, id_S)),
            getMSSAdress(id_S));
        stats.theoreticalNbWifiMessages++;
    }
}
...
void MH::Resend(cMessage* msg) {
    ...
    if (!state.isInitiatingConnection())
        sendPacket(new Packet("Apph_S", MessageFactory::createApph_S({id_h, elt.seqh}, id_S, Ses)), getMSSAdress(id_S));
