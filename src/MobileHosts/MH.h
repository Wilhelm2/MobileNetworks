...
// It implements the connection of hosts to stations as well as the sending, reception, delivering and acknowledgment of
...
class MH : public HostCommunications {
   public:
    ...
    // Checks every 100ms to connect to the closest station. Initiates a new connection whenever changes the station to
    ...
    // Initializes the host. The host is initialized her and not in constructor because it requires that INET is
    // initialized
    void processStart();
   ...
   public:
    ...
    unsigned int Ses = 0;
    // The session counter of the most recent session in which the host received a confirmation that the station
    // received its Init message ie, the most recent connection in which the host received an InitAck or AppCo message
    // from the station to which it connected itself
