...
class MHRBuffer {
   public:
    ...
    // The function is executed when the host receives an AppCo or InitAck message from the station to which it tries to
    // connect updatedSeqS contains the order with which the new station orders application messages
    ...
    // Contains the old buffer state before the host changed cell. The host keeps it till it receives the first AckCo or
    // InitAck message The purpose is to not loose the received but not delivered messages when changing cell
