...
void Communications::socketDataArrived(TcpSocket* socket, Packet* packet, bool urgent) {
    while (true) {
        ...
        if (dynamic_pointer_cast<const SliceChunk>(packet->peekAtFront()) != nullptr) {
            ...
            if (ReassembleBuffers.find(socket->getSocketId()) ==
                ReassembleBuffers.end())  // nouveau packet en plusieurs chunks
            ...
            if (ReassembleBuffers[socket->getSocketId()]->isComplete()) {
                handleReceivedMessage(dynamic_pointer_cast<const FieldsChunk>(
                                          ReassembleBuffers[socket->getSocketId()]->getReassembledData()),
                                      packet);
