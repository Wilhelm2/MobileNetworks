...
struct statsMSS {
    ...
    unsigned int theoreticalNbWifiMessages =
        0;  // par ex lorsque group 2 messages dans un send, ce sera bien considéré comme 2 sends ici
...
class Communications : public ApplicationBase, public UdpSocket::ICallback, public TcpSocket::ICallback {
   public:
    ...
    // Called upon reception of a packet. Messages can be split in several packets and packets may contain several
    // messages. Hence, messages must first be extracted from packets.
    ...
    // Sequence number used to process  messages sent over the wired network are processed exactly once and in FIFO
    // order
