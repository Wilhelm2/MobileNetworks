...
void MSS::handleReceivedMessage(cMessage* msg) {
    if (TcpSocket* socketMsg =
            check_and_cast_nullable<TcpSocket*>(MSSsocketMap.findSocketFor(msg)))  // New TCP connection
...
void MSS::handleTimeout(cMessage* msg) {
    if (TimeoutHandoff* tmp = dynamic_cast<TimeoutHandoff*>(msg)) {
        ...
        scheduleMessageLoadDependantWithPartialProportionnality(
            m, m->getSeq(), session->getCoMsgDeliver().front().seqNC, session->getCoMsgDeliver().back().seqNC);
...
template <typename T>
void MSS::sendMessage(IntrusivePtr<T> msg) {
...
void MSS::scheduleMessageLoadDependantWithPartialProportionnality(cMessage* m,
                                                                  unsigned int seq,
                                                                  unsigned int min,
                                                                  unsigned max) {
    double messageLoad = params->nbMobileHosts / params->AppMFrequency;
...
void MSS::ReceiveAckSS(const IntrusivePtr<const AckSS>& m) {
    if (MSSb.contains(m->getIdS())) {
        for (tuple<unsigned int, unsigned int, unsigned int> t : m->getVSeq()) {
            ...
            SBuffer.tagAlreadyDeliveredMessagesByMH(
                acknowledgedAppCo, get<0>(t));  // adds id of mh to those messages if they are stored in SBuffer
        }
    }
}
...
void MSS::ReceiveAppCoACK(const IntrusivePtr<const AppCoAck>& m) {
    if (Mobiles.isSessionRegistered(m->getId_h(), m->getSes())) {
        ...
        if (session->connectionInHandoff()) {
            ...
            sendMessage(
                MessageFactory::createInitAck(id, m->getId_h(), session->seqh, session->seqc, session->Ses, msgOrder));
        }
    }
}
...
void MSS::handleNewConnection(unsigned int id_h, const IntrusivePtr<const Init>& m) {
    if (Mobiles.hasSessionInCharge(id_h, id))  // another session of the host is registered
    {
        ...
        if (!session->isInHandoff()) {
            if (m->getSes_LC() == session->Ses)  // was connected to this station, moved but did not succeed to connect
                                                 // to another station, then came back
            {
                list<struct s_sendMsgMSS> acknowledgedAppCo = session->getCoMsgDeliver().clear(m->getSeq_S());
...
void MSS::RegisterHost(unsigned int id_h, const IntrusivePtr<const Init>& m) {
    Session h(id_h, 0, SBuffer.getMinSeqNC() - 1, {{0, 0}}, m->getSes(),
              id);  // getMinSeqNC()-1 to avoid the mss to delete any message during the handoff
...
void MSS::ReceiveApph_S(const IntrusivePtr<const Apph_S>& m) {
    if (Mobiles.isSessionRegistered(m->getId().idMH, m->getSes())) {
        ...
        if (session->notInInitState())  // ensures that mss won't broadcast a message from h_i before it received all
                                        // messages that h_i might have delivered when connected to another station
        {
...
void MSS::ReceiveApp_sresp(const IntrusivePtr<const App_resp>& m, Packet* sourcePacket) {
    if (MSSb.contains(m->getSourceId())) {
        // if previous mss failed and this mss is now responsible, then it has already broadcasted this message and does
        // only need to remove it (but should never happen !)
...
void MSS::ReceiveRecoverMSSReq(const IntrusivePtr<const RecoverMSSReq>& m) {
    if (!MSSb.contains(m->getRecoveringMSSId()))
        ...
        sendToStations(MessageFactory::createRecoverMSS(m->getRecoveringMSSId(), id, ++seqMsg, SBuffer,
                                                        SBuffer.getSeqC(), MSSb.getGroupResponsible(), FIFOorder,
                                                        Mobiles.Mobiles));
}
...
void MSS::ReceiveReq_1(const IntrusivePtr<const Req1>& m) {
    ...
    if (session->isInHandoff() ||
        session->Ses > m->getSes())  // session is currently in handoff or is more recent than the requester session
        return;

    list<struct s_sendMsgMSS> acknowledgedAppCo =
        session->updateProcessReq1(FIFOorder, m->getSes(), m->getSeq_S(), m->getSes_LC());

    SBuffer.tagAlreadyDeliveredMessagesByMH(acknowledgedAppCo, m->getId_h());
    cancelAndDeleteTimeouts(acknowledgedAppCo);
    sendToStations(MessageFactory::createRsp1(m->getId_h(), session->seqh, buildVectorIdsNotDeliveredMessages(*session),
                                              m->getSes(), id, ++seqMsg));
}
...
vector<idMsg> MSS::buildVectorIdsNotDeliveredMessages(const Session& session) {
    ...
    vector<idMsg> notDeliveredSBufferMessages =
        SBuffer.getIdNotDeliveredMessages(session.id, session.seqc);  // Not delivered messages in sending buffer
...
void MSS::ReceiveRsp_1(const IntrusivePtr<const Rsp1>& m) {
    ...
    sendToStations(MessageFactory::createReq2(m->getId_h(), SBuffer.notContainedMessages(m->getM_nd()), m->getSes(),
                                              MSSb, id, ++seqMsg));
}
...
void MSS::ReceiveReq_2(const IntrusivePtr<const Req2>& m) {
    ...
    sendToStations(MessageFactory::createRsp2(m->getId_h(), m->getMsg_req(),
                                              SBuffer.getIdMessagesReceivedAfter(session->recvReq1Vector), m->getSes(),
                                              id, ++seqMsg));
    // Mobiles.removeSession(id_h, m->getSes()); Will do it upon reception of DELETE. Otherwise the causal information
    // of host will be lost if the requesting station fails before sending the information to its backup stations
}
...
void MSS::ReceiveRsp_2(const IntrusivePtr<const Rsp2>& m) {
    ...
    cancelAndDelete(session->updateProcessRsp2(buildAppCoMsg(
        SBuffer.getIdMessagesNotDeliveredNorBroadcasted(m->getId_h()), m->getMsg(), m->getId_h(), m->getSes())));
    if (session->getCoMsgDeliver().empty())
        FinalizeInit(m->getId_h(), m->getSes());
    else
...
list<struct s_sendMsgMSS> MSS::buildAppCoMsg(const vector<idMsg>& alreadyAcknowledgedMessages,
                                             const vector<idMsg>& alreadyDeletedMessages,
                                             unsigned int id_h,
                                             unsigned int Ses) {
...
void MSS::sendAppCoMsg(const list<struct s_sendMsgMSS>& appCoMsg, unsigned int id_h, unsigned int Ses) {
    ...
    vector<tuple<idMsg, unsigned int>> messageOrder = buildMessageOrderList(
        appCoMsg);  // builds the list that contains the sequence number order of messages broadcasted by that Station
...
void MSS::UpdateResendPacket() {
    ...
    while (resendPacket->getTotalLength() != resendPacket->getFrontOffset()) {
        ...
        if (m && SBuffer.broadcastedMessagesContain(m->getSeq_S()))  // message might have been acknowledged by all mhs
                                                                     // since then and is thus not in SBuffer anymore
        {
