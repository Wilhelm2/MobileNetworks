...
class MSS : public Communications {
   public:
    ...
    template <typename T>
    void sendMessage(IntrusivePtr<T> msg);
    ...
    void scheduleMessageLoadDependantWithPartialProportionnality(cMessage* m,
                                                                 unsigned int seq,
                                                                 unsigned int min,
                                                                 unsigned max);
    ...
    // Checks whether broadcasted messages have been delivered by all connected hosts and can be stopped being
    // broadcasted
    ...
    // Handles a received recovery message - initializes the station, sets it to the up state and resumes normal
    // execution
