...
vector<TimeoutAppS_h*> MSSSBuffer::resendBroadcastedMessages(unsigned int id_h) {
    ...
    while (it != buffer.end() &&
           it->MHdelivContains(id_h))  // skips messages till the first that the mobile has not already delivered
        it++;

    // resends the others if they are not broadcasted (must resend them so that the mh can acknowledge them, which it
    // must even if it has already delivered them !)
...
unsigned int MSSSBuffer::determineSeqC(unsigned int id_h) const {
    ...
    while (it != buffer.buffer.end() && !it->timeout) {
        it++;  // jump the messages that the station might has already aknowledged to other stations (and which the
               // station sends as AppCoMsg if the host did not deliver them)
        min++;
    }
...
void MSSSBuffer::tagAlreadyDeliveredMessagesByMH(const list<struct s_sendMsgMSS>& acknowledgedAppCo,
                                                 unsigned int idMH) {
