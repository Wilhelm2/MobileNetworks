...
class MobileHostsMap {
   public:
    ...
    // Finds the session of idMH with the session sequence number Ses. Returns Mobiles[id_h].end() if the session is not
    // registered
    vector<Session>::iterator findSession(unsigned int idMH, unsigned int Ses);
    // REturns if registers a session of idMH that is not in handoff. Returns Mobiles[id_h].end() if no registered
    // session is not in handoff
    ...
    // Returns if registers a session of idMH of which the local station is in charge. Returns Mobiles[id_h].end() if
    // not
    ...
    // For each host idMH contains a vector with the connections of idMH with stations of the group of the current
    // station
