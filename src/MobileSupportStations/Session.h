...
public:
	...
	{
		Init, Rcv_Rsp1, Send_MD, Established, Rcv_Req1
	};
	Session();
	virtual ~Session();
	Session(unsigned int id, unsigned int seqh, unsigned int seqc, const vector<AckInterval>& ackIntervals,
			unsigned int Ses, unsigned int MSSinCharge);
	bool isInHandoff() const;
	// Updates the session upon processing a Req1 message
	list<struct s_sendMsgMSS> updateProcessReq1(vector<unsigned int>& recvReq1Vector, unsigned int SesReq1,
			unsigned int seqc, unsigned int Ses_LC);
	// Updates the session upon processing a Rsp1 message
	void updateProcessRsp1(unsigned int seqh, const vector<unsigned int>& recvRsp1Vector,
	...
	vector<AckInterval> ackIntervals; // Contains intervals of sequences in order to acknowledge messages more efficiently and broadcast them less on the wireless network
	StationReceiveBuffer RBuffer; // Reception buffer of application messages broadcasted by host id
	unsigned int Ses; // Session sequence number. Each session has an associated session sequence number given by the host id. Host id increments the session sequence number at each new connection it tries to establish
	Session::CONNECTION_STATE state = Session::CONNECTION_STATE::Init;

	unsigned int MSSinCharge = -1; // Station in charge of host id. Necessary since stations of a group keep a copy of the connection of hosts connected to other stations of the group
	...
	TimeoutHandoff* handoffTimeout; // Must be a pointer because MSS must own it since the timer cannot be cancelled in the Session function
	vector<idMsg> notDeliveredMessages; // Contains the id of not delivered messages, stored to be used upon reception of Rsp2

private:
	StationAppCoSendingBuffer coMsgToDeliver; // Contains the application messages the host must deliver before delivering application messages currently broadcasted by the station
};
