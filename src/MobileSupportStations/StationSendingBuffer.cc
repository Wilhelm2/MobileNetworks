...
bool StationSendingBuffer::containsMessage(idMsg IDMessage, const vector<idMsg>& msg_rcv,
		const vector<idMsg>& notDeliveredMessages)
{
	auto lambda_exp = [IDMessage] (const idMsg& id)
	{	return ID_EQ(id, IDMessage);};

	bool msg_rcvContains = find_if(msg_rcv.begin(), msg_rcv.end(), lambda_exp) != msg_rcv.end();
	bool notDelieredMessagesContains = find_if(notDeliveredMessages.begin(), notDeliveredMessages.end(), lambda_exp)
			!= notDeliveredMessages.end();
...
//			cout << "MH " << id_h << " has already delivered (" << it.id.idMH << "," << it.id.seqMH << ")" << " seqNC "
//					<< it.seqNC << " still broadcasted " << (it.timeout != nullptr) << endl;
		}
		else
			;
//			cout << "MH " << id_h << " has not delivered (" << it.id.idMH << "," << it.id.seqMH << ") seqNC "
//					<< it.seqNC << " still broadcasted " << (it.timeout != nullptr) << endl;
	}
}
