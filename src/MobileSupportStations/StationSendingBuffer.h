...
struct s_sendMsgMSS
{
	...
	// Must be kept as pointer because timeouts are stored in StationSendingSBuffer but are deleted/scheduled in the MSS class
	cMessage* timeout;
	...
	// Used as reception time of messages. A simtime_t variable is not used because two messages can be received at the same simetime_t.
	...
	s_sendMsgMSS(unsigned int seqNC, idMsg id, cMessage* timeout, vector<unsigned int> MHdeliv,
			vector<unsigned int> recvVector)
	{
		this->seqNC = seqNC;
		this->id = id;
...
public:
	...
	// Clears all the message from the buffer whose sequence number is lower than minSeq, acknowledges the messages to BC, and returns the timeouts to cancel
